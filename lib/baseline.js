...
/* Cache-key versioneren: bij een wijziging in compute() mag een oude cache
   (van vóór de hot-reload) nooit blijven hangen — dat gaf "—" op het
   dashboard terwijl het werkboek gewoon bestond. */
const CACHE_KEY = "__pocBaseline_v2";

export function getBaseline() {
  if (!globalThis[CACHE_KEY]) {
    try {
      globalThis[CACHE_KEY] = compute();
    } catch {
      globalThis[CACHE_KEY] = null; // export niet aanwezig (bv. repo zonder sources) — UI valt terug
    }
  }
