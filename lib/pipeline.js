...
export function precedentLibrary() {
  ...
  for (const p of store.precedents) {
    out.push({
      ...
      goederen: p.goederen,
      bron: p.afgewezen
        ? `CORRECTIE door declarant: het model stelde ${p.afgewezen} voor, de declarant koos ${p.hs}${p.reden ? ` — reden: "${p.reden}"` : ""} — weeg deze correctie zwaarder dan een gewoon precedent`
        : "live bevestigd door declarant",
      ...(p.afgewezen ? { afgewezen_voorstel: p.afgewezen } : {}),
    });
  }
  return out;
}
